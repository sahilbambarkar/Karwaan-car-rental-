...
const Booking = ({ searchParams }) => {
    ...
    return (
        <div className="pt-20">
                    ...
                    {steps?.map((item, index) => (
                        <div key={index}>
                            <div className="flex items-center gap-5">
                                ...
                                {index !== 3 && (
                                    <div
                                        className={`w-full h-0.5 ${current > index + 1 ? "bg-third-color" : "bg-black"
                                            } rounded`}
                                    ></div>
                                )}
                            </div>
            ...
            <div
                className={`${current == 4 ? "hidden" : "flex"
                    } justify-between items-center px-[10%] py-3`}
            >
                <Link
                    href={`/booking?index=${current - 1}`}
                    className="bg-slate-300 text-white px-10 py-3 rounded-full"
                >
                    back
                </Link>
                <Link
